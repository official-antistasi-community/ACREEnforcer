...
CLASS("TSE", "MessageReceiver")
	...
	METHOD_FILE("handleMessage", "x\TSE\addons\enforcer\TSE\handleMessage.sqf"); // Overwrite the base class method
	METHOD_FILE("handleClientState", "x\TSE\addons\enforcer\TSE\handleClientState.sqf"); // Checks the status of the client and performs appropriate actions on it	
	...
	METHOD("new") {
		...
		// Read settings
		private _settings = call compile preprocessFileLineNumbers "x\TSE\addons\enforcer\settings.sqf";
		diag_log format ["[TSE] Info: Settings: %1", _settings];
		...
		private _hThread = [_thisObject, TIMER_INTERVAL] spawn compile preprocessFileLineNumbers "x\TSE\addons\enforcer\TSE\timerThread.sqf"; // TSE object, timer interval
